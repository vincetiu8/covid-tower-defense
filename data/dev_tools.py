...
class DevClass(Game):
    ...
    def event(self, event):
        ...
        if name == "tower_name" or name == "tower_stage" or name == "enemy_name":
            return -3
...
class TowerPreview(DevClass):
    ...
    def load_ui(self):
        ...
        self.ui.new_attr(Attribute("description", {"type": "string"}, TOWER_DATA[self.tower_names[self.current_tower]]["description"]))

        for attr in ATTR_DATA["tower"]:
            self.ui.new_attr(Attribute(attr, ATTR_DATA["tower"][attr], TOWER_DATA[self.tower_names[self.current_tower]]["stages"][self.current_stage][attr]))
    ...
    def create_new_tower(self):
        ...
        for stage in range(3):
            ...
            for attr in ATTR_DATA["tower"]:
                TOWER_DATA[self.new_tower_name][stage][attr] = ATTR_DATA["tower"][attr]["default"]
            TOWER_DATA[self.new_tower_name][stage]["gun_image"] = pg.image.load(
                path.join(TOWERS_IMG_FOLDER, self.new_tower_name + "_gun" + str(stage) + ".png"))
            TOWER_DATA[self.new_tower_name][stage]["base_image"] = pg.image.load(
                path.join(TOWERS_IMG_FOLDER, self.new_tower_name + "_base" + str(stage) + ".png"))
            TOWER_DATA[self.new_tower_name][stage]["bullet_image"] = pg.image.load(
                path.join(TOWERS_IMG_FOLDER, self.new_tower_name + "_bullet" + str(stage) + ".png"))
            TOWER_DATA[self.new_tower_name][stage]["shoot_sound_path"] = path.join(TOWERS_AUD_FOLDER, "{}.wav".format(self.new_tower_name))
            temp_base = TOWER_DATA[self.new_tower_name][stage]["base_image"].copy()
            temp_base.blit(TOWER_DATA[self.new_tower_name][stage]["gun_image"],
                           TOWER_DATA[self.new_tower_name][stage]["gun_image"].get_rect(
                               center=TOWER_DATA[self.new_tower_name][stage]["base_image"].get_rect().center))
            TOWER_DATA[self.new_tower_name][stage]["image"] = temp_base
        self.tower_names = list(TOWER_DATA.keys())
