...
class Menu(Display):
    def __init__(self):
        ...
        self.tower_preview_button = pg.Rect((800, 100), self.level_button_rect.size)

        self.tower_edit_button = pg.Rect((1200, 100), self.level_button_rect.size)
        self.enemy_edit_button = pg.Rect((1200, 500), self.level_button_rect.size)
        self.level_edit_button = pg.Rect((1200, 900), self.level_button_rect.size)
...
class TowerSelectMenu(Display):
    ...
    def draw(self):
        ...
        self.blit(title_1, (
        SCREEN_WIDTH / 4 - title_1.get_width() / 2, 0))  # puts these on the x center of the screnn's left half
        self.blit(selected_text, (SCREEN_WIDTH / 4 - selected_text.get_width() / 2, title_1.get_height() - 20))
        ...
        if self.over_tower[0] != -1:
            ...
            if self.tower_rects[row][col].centerx < self.get_width() / 2:
                ...
                self.blit(self.tower_infos[ind],
                          self.tower_infos[ind].get_rect(topright=self.tower_rects[row][col].topleft))
    ...
    def draw_map(self, map):
        ...
        # scales map down so that it is no bigger than a rectangle with dimensions (SCREEN_WIDTH / 2, SCREEN_WIDTH / 4)
        scale_factor = min((SCREEN_WIDTH / 2 - GRID_MARGIN_X) / img.get_width(),
                           (SCREEN_HEIGHT / 2 - 40) / img.get_height())
        self.map_img = pg.transform.scale(img,
    ...
    def draw_wave_info(self):
        ...
        for sub_wave in self.level_data["waves"][self.curr_wave]:
            text = wave_font.render("{}x".format(sub_wave["enemy_count"]), 1, WHITE)
            enemy_img = pg.transform.scale(ENEMY_DATA[sub_wave["enemy_type"]]["image"],
                                           (GRID_2_CELL_SIZE, GRID_2_CELL_SIZE))
            enemy_surf = pg.Surface(
                (text.get_width() + enemy_img.get_width(), max(text.get_height(), enemy_img.get_height())))
